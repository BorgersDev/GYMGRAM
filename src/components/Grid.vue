    ...
    <section id="grid">
        <button :disabled="workoutIdx > 0 && workoutIdx > firstIncompleteWorkoutIndex " @click="() => handleSelectWorkout(workoutIdx)" :key="workoutIdx" v-for="(workout, workoutIdx) in Object.keys(workoutProgram)" class="card-button plan-card">
            <div>
                ...
                <i class="fa-solid fa-dumbbell" v-if="workoutIdx % 3 == 0"/>
                <i class="fa-solid fa-weight-hanging" v-if="workoutIdx % 3 == 1" />
                <i class="fa-solid fa-bolt" v-if="workoutIdx % 3 == 2" />
            </div>
            <h3> {{ workoutTypes[workoutIdx % 3] }}</h3>
        </button>
        <button @click="handleResetPlan" :disabled="firstIncompleteWorkoutIndex !== -1" class="card-button plan-card-reset">
            <p>Reset</p>
