<script setup>
  ...
  for(let workoutIdx in workoutProgram) {
    ...
    // creating a loop where we iterate over every workout
    defaultData[workoutIdx] = {} // initialize the workout data obj

    // nested loop to loop over every exercise within a workout and initialize it's 
    //input value to an empty string
  ...
  function handleResetPlan() {
    selectedDisplay.value = 2
    selectedWorkout.value = -1
    data.value = defaultData // reset the data to the default state
  ...
  onMounted(() => {
    ...
    if (localStorage.getItem('workouts')) {
      // only enter the if block if we find some data saved to the key workouts in the localstorage database
      const savedData = JSON.parse(localStorage.getItem('workouts'))
      data.value = savedData
      selectedDisplay.value = 2 // if they have data then we show them the dashboard
    }
  })
</script>
